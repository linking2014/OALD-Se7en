...
(function () {
  ...
  if (localStorage.autozoom === undefined) {
    ...
    localStorage.showVocabulary = "true";//生词表
    localStorage.showHistory = "true";//历史记录
  ...
  if (localStorage.showVocabulary == "false") {
    $("#vocabulary").addClass('hide')
  }

  $("#vocabulary > .closeme, #history > .closeme").click(function (e) {
    var name = $(this).parent()[0].id
    var name2 = name[0].toUpperCase() + name.slice(1)//首字母大写
    if (localStorage['show' + name2] == "false") {
      localStorage['show' + name2] = "true"
      $(this).parent().removeClass('hide')
    } else {
      localStorage['show' + name2] = "false"
      $(this).parent().addClass('hide')
    }
  })

  if (localStorage.showHistory == "false") {
  ...
  $("body").keyup(function () {
    ...
    switch (window.event.keyCode) {
      ...
      case 80: //p,pronunciation
        // pronunciation.play();
        GoogleTTS(word).then(playSoundUrl)
        break;
      case 82: //r,reload
        location.reload();
        break;
  ...
  $content.on("mousedown", "span.oa_d,span.oa_ud,span.oa_x", function () {
    ...
    console.log('准备阅读:', soundtext)


    GoogleTTS(soundtext).then(url => {
      playSoundUrl(url)
