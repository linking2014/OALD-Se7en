$(function () {
  $("#export-button").click(function () {
    ...
    db.sql('SELECT word,time,content FROM vocabulary ,oald7 WHERE word = headword ORDER BY time DESC', [], function (tx, result) {
      for (var i = 0; i < result.rows.length; i++) {
        var $result = $("#result");
        var source = result.rows.item(i)['source'];
        var word = result.rows.item(i)['word'];
        var wordcontent = result.rows.item(i)['content'];
        var chnmeanning = $(wordcontent).find(".oa_d>.oa_chn,.oa_ud>.oa_chn").map(function () {
          return $(this).text();
        }).get().join("<br>");
        var pron = "/%s/".format($(wordcontent).find("span.oa_i_phon").text());
        $result.append('<tr><td style="color:blue;">%s %s</td></tr><tr><td>%s</td></tr>'.format(word, pron, chnmeanning));
      }
    });
  });
